  ...
  try {
    for (let i = 0; i < lists.length; i++) {
      ...
      await sb.from('task_lists').upsert({
        ...
        is_default: l.isDefault, position: i,
...
export async function addList(name: string): Promise<TaskList> {
  const newList: TaskList = { id: crypto.randomUUID(), name, isDefault: false, createdAt: Date.now() };
  ...
  if (sb && userId) {
    try {
      await sb.from('task_lists').insert({
        id: newList.id, user_id: userId, name,
        is_default: false, position: lists.length - 1,
  ...
  try {
    for (const t of tasks) {
      await sb.from('tasks').upsert({
        id: t.id, user_id: userId, list_id: t.listId,
        title: t.title, notes: t.notes || null,
        repeat: t.repeat, start_time: t.startTime || null,
        end_time: t.endTime || null, alarm: t.alarm,
        ringtone: t.ringtone || null, completed: t.completed,
        completed_at: t.completedAt || null, created_at: t.createdAt,
      }, { onConflict: 'id' });
    }
  } catch {}
}
...
export async function addTask(task: Omit<Task, 'id' | 'createdAt' | 'completed'>): Promise<Task> {
  const newTask: Task = { ...task, id: crypto.randomUUID(), completed: false, createdAt: Date.now() };
  ...
  if (sb && userId) {
    try {
      await sb.from('tasks').insert({
        id: newTask.id, user_id: userId, list_id: newTask.listId,
        title: newTask.title, notes: newTask.notes || null,
        repeat: newTask.repeat, start_time: newTask.startTime || null,
        end_time: newTask.endTime || null, alarm: newTask.alarm,
        ringtone: newTask.ringtone || null, completed: false,
        completed_at: null, created_at: newTask.createdAt,
      });
    } catch {}
  }
  return newTask;
...
export async function seedIfEmpty(): Promise<void> {
  ...
  const seeded = DEFAULT_TASKS.map((t) => ({
    ...t, id: crypto.randomUUID(), completed: false, createdAt: Date.now(),
  }));
...
async function syncToSupabase(): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < lists.length; i++) {
      await sb.from('task_lists').upsert({
        id: lists[i].id, user_id: userId, name: lists[i].name,
        is_default: lists[i].isDefault, position: i,
