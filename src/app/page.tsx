...
import { getLists, getTasks, saveTasks, addList, deleteList, addTask, deleteTask, updateTask, seedIfEmpty, setCurrentUserId } from '@/lib/store';
...
export default function Home() {
  ...
  const taskCounts = lists.reduce(
    (acc, list) => {
      acc[list.id] = tasks.filter((t) => t.listId === list.id && !t.completed).length;
      return acc;
    },
    {} as Record<string, number>
  );

  const getActiveListName = () => lists.find((l) => l.id === activeListId)?.name || 'Tasks';

  const getVisibleTasks = () => {
    const incomplete = tasks.filter((t) => t.listId === activeListId && !t.completed);
    const completed = tasks.filter((t) => t.listId === activeListId && t.completed);
  ...
  const handleAddTask = async (data: {
    listId: string; title: string; notes?: string;
    repeat: RepeatType; startTime?: string; endTime?: string;
    alarm: boolean; ringtone?: string;
  }) => {
    ...
    return (
      <div className="min-h-screen bg-[#05080c] flex items-center justify-center">
        <div className="text-center animate-fade-in">
          ...
          <p className="text-xs text-neon-green tracking-[0.3em] uppercase" style={{ fontFamily: 'Orbitron, monospace' }}>
            Loading...
          </p>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#05080c] alien-grid flex flex-col lg:flex-row">
      ...
      <main className="flex-1 min-h-screen lg:h-screen lg:overflow-hidden flex flex-col">
        <header className="flex items-center justify-between p-5 lg:p-8 pb-0 lg:pb-0">
          <div>
            <h1 className="text-xl lg:text-2xl font-bold tracking-wider uppercase" style={{ fontFamily: 'Orbitron, monospace' }}>
              {getActiveListName()}
            </h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2.5 rounded-xl border border-[rgba(255,59,92,0.2)] text-[#606060] hover:text-neon-red hover:border-[rgba(255,59,92,0.4)] transition-all"
              title="Sign out"
            >
